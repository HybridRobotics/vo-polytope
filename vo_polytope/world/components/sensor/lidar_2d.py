...
from vo_polytope.util import range_seg_matrix, range_cir_seg, range_seg_seg
import random
...
class lidar2d:
    ...
    def seg_components(self, segment, components):
        ...
        for robot in components["robots"].robot_list:
            ...
            flag, int_point, lrange = range_cir_seg(
                robot.state[0:2, 0], robot.radius, segment
            )
        ...
        for obs_cir in components["obs_circles"].obs_cir_list:
            flag, int_point, lrange = range_cir_seg(
                obs_cir.state[0:2, 0], obs_cir.radius, segment
            )
        ...
        flag, int_point, lrange = range_seg_matrix(
            segment,
            components["map_matrix"],
            components["xy_reso"],
            self.point_step_weight,
            components["offset"],
        )
