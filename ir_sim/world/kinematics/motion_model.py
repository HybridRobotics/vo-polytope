...
def motion_diff(
    current_state, vel, sampletime, noise=False, alpha=[0.01, 0, 0, 0.01, 0, 0]
):
    ...
    if noise == True:
        ...
        vel_noise = vel + np.random.normal(
    ...
    if omegat >= 0.01 or omegat <= -0.01:
        ratio = vt / omegat
        next_state = current_state + np.array(
            [
                [-ratio * sin(theta) + ratio * sin(theta + omegat * sampletime)],
        ...
        next_state = current_state + np.array(
            [[vt * sampletime * cos(theta)], [vt * sampletime * sin(theta)], [0]]
        )
...
def motion_omni(current_state, vel, sampletime, noise=False, control_std=[0.01, 0.01]):
    ...
    if noise == True:
        vel_noise = vel + np.random.normal(
            [[0], [0]], scale=[[control_std[0]], [control_std[1]]]
        )
    else:
        vel_noise = vel
...
def motion_ackermann(
    ...
    if ack_mode == "default":
        ...
        co_matrix = np.array(
            [[cos(phi), 0], [sin(phi), 0], [tan(psi) / wheelbase, 0], [0, 0]]
        )
        d_state = co_matrix @ vel
        new_state = state + d_state * step_time
...
def motion_acker_pre(
    state, wheelbase=1, vel=1, psi=0, steer_limit=pi / 4, pre_time=2, time_step=0.1
):
    ...
    while cur_time < pre_time:
        phi = state[2, 0]
        d_state = np.array(
            [[vel * cos(phi)], [vel * sin(phi)], [vel * tan(psi) / wheelbase], [0]]
        )
...
def motion_acker_step(
    state, gear=1, steer=0, step_size=0.5, min_radius=1, include_gear=False
):
    ...
    rot_matrix = np.array(
        [[cos(rot_theta), -sin(rot_theta)], [sin(rot_theta), cos(rot_theta)]]
    )
    ...
    if include_gear:
        new_state = np.zeros((4, 1))
        new_state[0:2] = np.around(
            rot_matrix @ (state[0:2] - center) + center + trans_matrix, 4
        )
