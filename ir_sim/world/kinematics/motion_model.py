...
def motion_diff(current_state, vel, sampletime, noise=False, alpha=None):
    """ return the next state and the travel distance """
    ...
    if omegat >= 0.01 or omegat <= -0.01:
        ...
        next_state = current_state + np.array([[-ratio * sin(theta) + ratio * sin(theta + omegat * sampletime)],
                                               ...
                                               [omegat * sampletime]])
        distance = omegat * sampletime * ratio
...
def motion_omni(current_state, vel, sampletime, noise=False, control_std=None):
    """ return the next state """
    # vel: np.array([[vel x], [vel y]])
    # current_state: np.array([[x], [y]])

    if control_std is None:
        control_std = [0.01, 0.01]
    if noise:
...
def motion_ackermann(state, wheelbase=1, vel=np.zeros((2, 1)), steer_limit=pi / 2, step_time=0.1, ack_mode="default",
    ...
    if ack_mode == "default":
        ...
        # w is the rotation rate, dot(phi) = w
        new_state = np.zeros((4, 1))
...
def wraptopi(radian):
    """ convert the radian to the scope of (-pi, pi) """
    if radian > pi:
        radian2 = radian - 2 * pi
    elif radian < -pi:
        radian2 = radian + 2 * pi
